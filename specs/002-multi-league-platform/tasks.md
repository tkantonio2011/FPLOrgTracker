...
- [X]  T074 [P] [US3] Create `src\app\(main)\platform\audit\page.tsx` — full platform-wide audit feed; sourced from `GET /api/platform/audit`. **Done**: 50/page paginated feed. Filter row: action (exact-match input — operator can type e.g. `league.suspended`), "Since" datetime-local input (converted to ISO before send). League column links to `/platform/leagues/{id}` when the league still exists; falls back to a truncated id when the league has been deleted (so historical events with `leagueId=null` after a `league.deleted` cascade show "—"). Details column uses the same `<details>` JSON expander as the league-scoped audit page.
...
- [X]  T082 [P] [US3] Create `src\app\api\platform\audit\route.ts` (GET) per `platform-contracts.md`. **Done**: gates on `requireSuperAdmin`. Query supports `?page`, `?limit`, `?since` (ISO), `?actorUserAccountId`, `?leagueId`, `?action`. Newest-first. Row shape extends the league-scoped audit shape with `leagueName` + `leagueSlug` joined from the League row (null when the league has been deleted) so the platform audit page can render clickable league links without an N+1.

### Tests for US3

- [X]  T083 [P] [US3] Create `tests\integration\super-admin-guards.test.ts` (Vitest): League Admin and Member both receive 403 from every `/api/platform/...` endpoint. Bootstrap-env-only Super Admin succeeds. **Done (Prisma-level)**: 9 tests covering the data invariants `requireSuperAdmin` (and the underlying `getServerUser` derivation) checks. Helper `evaluateGate(userAccountId)` replicates the route-side decision tree: null → unauthenticated (401), disabled account → unauthenticated (getServerUser returns null), no/revoked SuperAdmin row → forbidden (403), active SuperAdmin row → allowed. Covers: League Admin role does NOT confer platform access; plain member does not; bootstrap path (upsert UserAccount + SuperAdmin) flips the gate idempotently; re-granting after revoke clears `revokedAt`. Mirrors `admin-role-guard.test.ts` and `last-admin-guard.test.ts`.
...
- [X]  T087 [P] [US4] Audit `CHANGELOG.md` — keep historical entries but ensure the running header is platform-generic. Past tense references to the original deployment can stay; future-tense copy must be neutral. **Done**: header reads "All notable changes to the FPL Organisation Tracker are documented here." — platform-generic. Past-tense entries (v1.5.0 "EnergyOne-themed verdicts", v1.6.2 "energy market report", v1.6.3 "energy trading archetypes") describe what existed at that point in time and are intentionally preserved as historical record. No future-tense industry copy remains.
- [X]  T088 [P] [US4] Audit `src\app\(main)\members\[managerId]\page.tsx` (now under `l\[leagueSlug]\` from T045) for any leftover company string; same for any `src\app\(main)\admin\page.tsx` legacy file (delete per T056). **Done**: `grep -i "energy.trading|EnergyOne"` returns zero matches under `src/`. T048 already stripped these from the member page footer and legacy admin page placeholder. The branding scan test in T091 enforces this going forward.
- [X]  T089 [P] [US4] Audit `src\app\(auth)\login\page.tsx` and `register\page.tsx` — these are deleted by T028; verify nothing else imports them or references their copy. **Done**: both directories deleted in T028. `grep -r "(auth)/login\|(auth)/register"` under `src/` returns zero matches — no leftover imports or references. Middleware no longer treats `/login` or `/register` as public prefixes; both URLs redirect to `/sign-in`.
- [X]  T090 [US4] Update `src\app\layout.tsx` (root) — page `<title>`, `<meta name="description">`, OG tags, favicon to platform-generic; never bake in a specific league name (per-league branding lives in `l\[leagueSlug]\layout.tsx`). **Done**: root metadata reads `{ title: "FPL Tracker", description: "Track Fantasy Premier League progress across multiple leagues." }`. OG tags + favicon not set — fall back to the auto-generated Next.js favicon and the title-derived OG, both platform-neutral. Per-league branding is handled at the league layout (Nav + AppShell consume `useLeague()` for the league name).
- [X]  T091 [US4] Add an automated branding scan: a Vitest unit test under `tests\unit\branding\no-industry-references.test.ts` that recursively reads `src\` and fails if any of `["energy trading", "EnergyOne", "energy.trading"]` (case-insensitive) appears in `.tsx`, `.ts`, `.css`, or `.md` files outside `tests/`, `specs/`, `node_modules/`, `CHANGELOG.md` historical sections. This is the SC-004 verifier. **Done**: scan recursively walks `src/` and matches three regex patterns: `/energy\s+trading/gi`, `/energy[^a-z0-9]trading/gi` (catches "energy.trading", "energy-trading", "energy_trading", etc.), and `/energyone/gi`. Reports file/line/column for every hit; throws a single error listing all of them. Test currently passes (0 hits).

### Migration validation

- [X]  T092 [US4] Create `tests\integration\migration.test.ts` (Vitest): seeds an in-memory SQLite DB with the *old* schema and a representative dataset (1 Organisation, 5 Members with mixed `source` values, 2 Users with `passwordHash`), then runs the migration in T024 in-process, then asserts: 1 League, 5 LeagueMemberships (3 with linked UserAccount, 2 without per email-presence), 0 `_legacy_*` rows visible to Prisma client, 1 SuperAdmin (per env), 1 admin LeagueMembership (per env), AuditEvent of `migration.completed` exists. Run the seed twice and assert no duplicates. **Done**: 2 tests against a temp SQLite DB. Test 1 seeds 1 Organisation + 5 Members (3 with email, 2 without; mixed source league/manual) + 2 Users with passwordHash, then runs the seed as a subprocess (`npx tsx seed.ts`) with `DATABASE_URL`/`BOOTSTRAP_SUPER_ADMIN_EMAIL`/`BOOTSTRAP_LEAGUE_ADMIN_EMAIL` injected. Asserts: 1 Platform row, 1 League, 5 LeagueMemberships (3 with userAccountId, 2 without), exactly 1 admin matching `BOOTSTRAP_LEAGUE_ADMIN_EMAIL`, lastLoginAt carried over for matching managerIds, 1 SuperAdmin matching `BOOTSTRAP_SUPER_ADMIN_EMAIL`, both `league.created` and `migration.completed` audit events present, source values preserved, and **legacy rows left intact** (Organisation/Member/User still readable — deviates from the original task description, which assumed `_legacy_*` rename; the implementation is expand-only). Test 2 re-runs the seed and asserts all counts unchanged. **Side-effect bug fix**: discovered that `migrateOrg`'s idempotency check (slug uniqueness) was broken — `generateSlug` always picks an unused slug, so re-runs never tripped the early-return and tried to create a duplicate League (failing on the `mini_league_id` unique constraint). Fixed by looking up the `migration.completed` AuditEvent for `details.legacyOrgId === org.id` instead.
- [X]  T093 [US4] Document the migration runbook in `specs\002-multi-league-platform\quickstart.md` (the file already exists and includes step-by-step migration day instructions — verify accuracy after T024 and T092 are complete; update if the script's exact prompts differ). **Done**: section 6 ("Migration Day") updated — replaced `npx prisma migrate deploy` with `npx prisma db push --skip-generate --accept-data-loss` to match the expand-only strategy (no migration history exists in the repo). Added a sentence noting the seed's idempotency detection now uses the `migration.completed` AuditEvent's `legacyOrgId`. Cross-link to `prisma/migrations/002_multi_league/rollback.md` added to step 1 (backup).
- [X]  T094 [P] [US4] Add a `prisma\migrations\002_multi_league\rollback.md` — a one-page document explaining that rollback is via the pre-migration backup and the steps to restore (per `research.md` Topic 6). Keep the SQLite backup pattern (`cp dev.db dev.db.pre-002`) explicit. **Done**: rollback doc explains the expand-only strategy means restore-from-backup is the only safe rollback (no reverse-migration script). Step-by-step: stop process, copy `dev.db.pre-002` over `dev.db`, redeploy previous build, restart, confirm. Covers the "no backup" case with three fall-back options (roll forward, reconstruct from still-present legacy tables via the 001 build, off-site backup). Also documents when the backup can be deleted (after the contract migration, which is itself a future task — once it lands, rollback is no longer possible).

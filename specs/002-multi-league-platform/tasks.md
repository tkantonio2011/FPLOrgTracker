...
- [X]  T053 [P] [US2] Create `src\app\(main)\l\[leagueSlug]\admin\settings\page.tsx` — form to edit name, slug, logo, mini-league ID, digest prompt. Wires to `PATCH /api/leagues/{id}` and `POST /api/leagues/{id}/sync`. **Done (PATCH only)**: form supports name, slug (with regex hint about old URLs continuing to work), logoUrl, miniLeagueId, digestPrompt. Diffs against the loaded server state to send only changed fields. Inline ok/error feedback. **Sync button intentionally omitted** because `POST /api/leagues/{id}/sync` (T058) is still pending — will land in the same batch as T058. A bare-`/admin` index page redirects to `/admin/settings` so the tab nav has a sensible default landing.

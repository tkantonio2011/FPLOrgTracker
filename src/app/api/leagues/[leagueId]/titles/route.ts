...
const TITLE_DEFS: TitleDef[] = [
  { id: "league_leader", title: "The League Leader", describe: (pts) => `Top of the league with ${pts} pts`, colour: "bg-yellow-100 text-yellow-800", border: "border-yellow-300", competitive: true },
  { id: "rocket", title: "The Rocket", describe: (pts) => `Season-high single GW score: ${pts} pts`, colour: "bg-orange-100 text-orange-800", border: "border-orange-300", competitive: true },
  { id: "bench_billionaire", title: "The Bench Billionaire", describe: (pts) => `${pts} pts left rotting on the bench — a national tragedy`, colour: "bg-amber-100 text-amber-800", border: "border-amber-300", competitive: true },
  { id: "gambler", title: "The Gambler", describe: (pts) => `Paid ${pts} pts in transfer hits — YOLO`, colour: "bg-red-100 text-red-800", border: "border-red-300", competitive: true },
  { id: "tinker_man", title: "The Tinker Man", describe: (n) => `Made ${n} transfers this season — can't stop, won't stop`, colour: "bg-violet-100 text-violet-800", border: "border-violet-300", competitive: true },
  { id: "fossil", title: "The Fossil", describe: (n) => `Set & forget — ${n} GWs without touching the squad`, colour: "bg-stone-100 text-stone-700", border: "border-stone-300", competitive: true },
  { id: "bottler", title: "The Bottler", describe: (n) => `Propped up the league leaderboard ${n} times — we appreciate the sacrifice`, colour: "bg-blue-100 text-blue-800", border: "border-blue-300", competitive: true },
  { id: "consistent", title: "Mr. Consistent", describe: (sd) => `GW-score std dev of just ${(sd / 10).toFixed(1)} — boring but effective`, colour: "bg-slate-100 text-slate-700", border: "border-slate-300", competitive: true },
  { id: "efficient", title: "The Efficient Machine", describe: (pts) => `Only ${pts} pts wasted on the bench — ruthless`, colour: "bg-emerald-100 text-emerald-800", border: "border-emerald-300", competitive: true },
  { id: "safe_hands", title: "Safe Pair of Hands", describe: () => `Not a single transfer hit taken all season`, colour: "bg-teal-100 text-teal-800", border: "border-teal-300", competitive: false },
];
...
const FALLBACKS = [
  ...
  { title: "The Average Joe", describe: () => "The league average, personified", colour: "bg-slate-100 text-slate-600", border: "border-slate-300" },
...
export async function GET(req: NextRequest, ctx: { params: { leagueId: string } }) {
  try {
    ...
    if (memberships.length === 0) return ok({ titles: [] as ManagerTitle[] });
    ...
    const raw = await Promise.all(
      memberships.map(async (m) => {
        ...
        const totalPoints =
          played.length > 0
            ? played[played.length - 1].total_points - m.pointsDeductionPerGw * played.length
            : 0;
    ...
    for (const gw of allGws) {
      ...
      for (const r of gwRow) {
        if (r.pts === minPts) gwsFinishedLast.set(r.id, (gwsFinishedLast.get(r.id) ?? 0) + 1);
      }
    }
    ...
    const sortedBy = {
      ...
      bottler: [...raw].sort((a, b) => (gwsFinishedLast.get(b.managerId) ?? 0) - (gwsFinishedLast.get(a.managerId) ?? 0)),
      consistent: [...raw].sort((a, b) => a.scoreStdDev - b.scoreStdDev),
      efficient: [...raw].sort((a, b) => a.totalBenchPts - b.totalBenchPts),
    } as const;

    const assigned = new Map<number, ManagerTitle>();
    ...
    return ok({ titles: Array.from(assigned.values()) });
  } catch (err) {

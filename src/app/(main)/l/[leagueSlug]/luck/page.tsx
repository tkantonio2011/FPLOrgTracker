...
function LuckStackedChart({ managers }: { managers: LuckManager[] }) {
  ...
  const leagueTotals = (["captainLuck", "benchLuck", "autoSubLuck"] as const).map((key) => ({
    key,
    ...
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      <div className="px-4 pt-4 pb-3 border-b border-slate-100 flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-3 flex-wrap shrink-0">
          {leagueTotals.map(({ key, label, colour, total }) => (
      ...
      <div className="px-2 py-3">
        <ResponsiveContainer width="100%" height={chartHeight}>
          <BarChart layout="vertical" data={chartData} margin={{ top: 0, right: 24, left: 8, bottom: 0 }} barCategoryGap="25%">
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
            <XAxis type="number" domain={[-domainBound, domainBound]} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} tickFormatter={(v) => (v > 0 ? `+${v}` : `${v}`)} />
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="bg-emerald-600 rounded-xl px-5 py-4 text-white shadow-card">
        ...
        <p className="text-3xl font-black tabular-nums mt-3 leading-none">+{lb.totalLuck}</p>
        ...
        <div className="mt-3 space-y-1 text-xs text-emerald-100">
          {lb.captainLuck > 0 && <p>⚽ Captains over-delivered by <strong>{sign(lb.captainLuck)} pts</strong> vs league avg</p>}
          {lb.captainHauls > 0 && <p>🎯 {lb.captainHauls} captain haul{lb.captainHauls !== 1 ? "s" : ""} (≥15 pts)</p>}
          {lb.benchLuck > 0 && <p>🪑 Bench was <strong>{sign(lb.benchLuck)} pts</strong> better than league avg</p>}
          {lb.autoSubLuck > 0 && <p>🔄 Auto-subs contributed <strong>+{lb.autoSubPts} pts</strong></p>}
        </div>
      </div>
      <div className="bg-red-700 rounded-xl px-5 py-4 text-white shadow-card">
        ...
        <p className="text-3xl font-black tabular-nums mt-3 leading-none">{ub.totalLuck}</p>
        <p className="text-red-200 text-xs mt-0.5">luck pts below average</p>
        <div className="mt-3 space-y-1 text-xs text-red-100">
          {ub.captainLuck < 0 && <p>⚽ Captains under-delivered by <strong>{sign(ub.captainLuck)} pts</strong> vs league avg</p>}
          {ub.captainBlanks > 0 && <p>💀 {ub.captainBlanks} captain blank{ub.captainBlanks !== 1 ? "s" : ""} (≤2 pts)</p>}
          {ub.benchLuck < 0 && <p>🪑 Bench over-scored by <strong>{Math.abs(ub.benchLuck)} pts</strong> vs league avg</p>}
          {ub.autoSubLuck < 0 && <p>🔄 Auto-subs underperformed by <strong>{ub.autoSubLuck} pts</strong></p>}
        </div>
      </div>
    </div>
  );
...
function ManagerCard({ manager, max, min }: { manager: LuckManager; max: number; min: number }) {
  ...
  const components = [
    {
      label: "Captain luck",
      value: b.captainLuck,
      detail: `You: ${b.captainTotal} pts · League avg: ${b.leagueAvgCaptainTotal} pts`,
    ...
    },
    {
      label: "Bench luck",
      value: b.benchLuck,
      detail: `Your bench: ${b.benchTotal} pts · League avg: ${b.leagueAvgBenchTotal} pts`,
      sub: null,
    },
    {
      label: "Auto-sub luck",
      value: b.autoSubLuck,
      detail: `Your auto-subs: ${b.autoSubPts} pts · League avg: ${b.leagueAvgAutoSubPts} pts`,
      sub: null,
    },
  ];
    ...
    <div className="px-4 py-4">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shrink-0 ${manager.rank === 1 ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-500"}`}>
            {manager.rank}
          </div>
        ...
        <div className="shrink-0 text-right">
          <p className={`text-lg font-black tabular-nums leading-none ${colours.text}`}>{sign(b.totalLuck)}</p>
          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full border mt-1 inline-block ${colours.badge}`}>{label}</span>
        </div>
      </div>
      <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden mb-3">
        <div className={`h-full rounded-full ${colours.bar} transition-all`} style={{ width: `${barPct}%` }} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {components.map(({ label: lbl, value, detail, sub }) => (
          <div key={lbl} className="bg-slate-50 rounded-lg px-3 py-2 border border-slate-100" title={detail}>
            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">{lbl}</p>
            <p className={`text-sm font-black tabular-nums mt-0.5 ${value > 0 ? "text-emerald-600" : value < 0 ? "text-red-500" : "text-slate-400"}`}>{sign(value)} pts</p>
...
export default function LuckPage() {
  ...
  const { data, isLoading, isError } = useQuery<LuckResponse>({
    queryKey: ["luck", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/luck`);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs text-slate-500 space-y-1">
        <p><span className="font-semibold text-slate-600">Captain luck</span> — actual captain pts vs league-average captain pts each GW (above avg = lucky)</p>
        <p><span className="font-semibold text-slate-600">Bench luck</span> — league-average bench pts vs your bench pts each GW (below avg = lucky: your bench players didn't outscore your XI)</p>
        <p><span className="font-semibold text-slate-600">Auto-sub luck</span> — pts scored by your auto-subbed players vs league average (more pts = lucky: your starters blanked but the subs delivered)</p>
      </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="px-4 py-4 border-b border-slate-50 last:border-0 space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">{[1, 2, 3].map((j) => <div key={j} className="h-12 bg-slate-100 rounded-lg animate-pulse" />)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {managers.length > 0 && (
        <>
          <HeroBanners luckiest={managers[0]} unluckiest={managers[managers.length - 1]} />
          ...
          <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
            ...
            <div className="divide-y divide-slate-50">
              {managers.map((m) => <ManagerCard key={m.managerId} manager={m} max={max} min={min} />)}
            </div>
          </div>
          <p className="text-xs text-slate-400 text-center px-2">
            Luck scores are relative to the league average — zero means exactly average luck. Hover component cards for raw numbers.
          </p>
        </>
      )}
    </div>

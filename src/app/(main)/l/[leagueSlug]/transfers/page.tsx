...
function PlayerPill({ player, direction }: { player: PlayerRef; direction: "in" | "out" }) {
  const colour = direction === "in" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-red-50 border-red-200 text-red-700";
  return (
...
function PopularMoveRow({ move, totalMembers, direction }: { move: PopularMove; totalMembers: number; direction: "in" | "out" }) {
    ...
    <div className="flex items-center gap-3 py-2">
      <div className="flex items-center gap-1.5 flex-1 min-w-0">
        ...
        {isDifferential && direction === "in" && (
          <span className="text-[9px] font-bold bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded-full uppercase tracking-wide shrink-0">Diff</span>
        )}
        {isCrowd && direction === "in" && (
          <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full uppercase tracking-wide shrink-0">Crowd</span>
        )}
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <div className="hidden sm:block w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div className={`h-full rounded-full ${direction === "in" ? "bg-emerald-400" : "bg-red-400"}`} style={{ width: `${pct}%` }} />
        </div>
        <span className="text-xs font-semibold text-slate-600 tabular-nums w-12 text-right">{move.count}/{totalMembers}</span>
      </div>
    </div>
  );
}
...
export default function TransfersPage() {
  ...
  const { data, isLoading, isError } = useQuery<TransfersResponse>({
    queryKey: ["transfers", league.id, selectedGw],
    queryFn: async () => {
      const url = selectedGw
        ? `/api/leagues/${league.id}/transfers?gw=${selectedGw}`
        : `/api/leagues/${league.id}/transfers`;
            ...
            <div className="relative">
              ...
              >
                {data.availableGws.map((gw) => (
                  <option key={gw.id} value={gw.id}>{gw.name}{gw.isCurrent ? " ★" : ""}</option>
                ))}
              </select>
        ...
        <div className="space-y-4">
          {[1, 2].map((i) => (
            <div key={i} className="bg-white border border-slate-200/80 rounded-xl p-4 animate-pulse shadow-card">
              ...
              <div className="space-y-3">{[1, 2].map((j) => <div key={j} className="h-10 bg-slate-100 rounded" />)}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <SummaryCard label="Transfers made" value={String(totalTransfers)} sub={`across ${managersWithTransfers.length} manager${managersWithTransfers.length !== 1 ? "s" : ""}`} />
            <SummaryCard label="Points hits" value={totalHits > 0 ? `-${totalHits}` : "None"} sub={totalHits > 0 ? "total deduction" : "all free transfers"} danger={totalHits > 0} />
            <SummaryCard label="Crowd picks" value={String(crowdPicks.length)} sub={crowdPicks.length > 0 ? crowdPicks.slice(0, 2).map((p) => p.name).join(", ") : "no consensus"} />
            <SummaryCard label="Differentials" value={String(differentials.length)} sub={differentials.length > 0 ? "unique transfers in" : "everyone aligned"} highlight={differentials.length > 0} />
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
                ...
                <div className="px-4 divide-y divide-slate-50">
                  {data.popularIns.map((move) => <PopularMoveRow key={move.playerId} move={move} totalMembers={data.totalMembers} direction="in" />)}
                  {data.popularIns.length === 0 && <p className="py-6 text-sm text-slate-400 text-center">No transfers in</p>}
                </div>
              </div>
              <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
                <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/60">
                ...
                <div className="px-4 divide-y divide-slate-50">
                  {data.popularOuts.map((move) => <PopularMoveRow key={move.playerId} move={move} totalMembers={data.totalMembers} direction="out" />)}
                  {data.popularOuts.length === 0 && <p className="py-6 text-sm text-slate-400 text-center">No transfers out</p>}
                </div>
              </div>
            </div>
          )}
                ...
                {managersWithTransfers.map((m) => (
                  <div key={m.managerId} className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
                    <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-500">{m.transfers.length} transfer{m.transfers.length !== 1 ? "s" : ""}</span>
                        {m.transferCost > 0 && <span className="text-xs font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded-full">-{m.transferCost} pts hit</span>}
                        {m.transferCost === 0 && m.transfers.length > 0 && <span className="text-xs font-medium bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full">Free</span>}
                      </div>
                    </div>
                        ...
                        return (
                          <div key={idx} className="px-4 py-3 flex items-center gap-2 sm:gap-3 flex-wrap sm:flex-nowrap">
                            ...
                            <div className="flex items-center gap-1.5 ml-auto">
                              {isDiff && <span className="text-[9px] font-bold bg-violet-100 text-violet-700 px-2 py-1 rounded-full uppercase tracking-wide whitespace-nowrap">Differential</span>}
                              {isCrowd && <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-2 py-1 rounded-full uppercase tracking-wide whitespace-nowrap">Crowd pick</span>}
                            </div>
                          </div>
                        );
                      })}
            ...
            <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
              ...
              <div className="px-4 py-2 flex flex-wrap gap-2">
                {managersNoTransfers.map((m) => (
                  <span key={m.managerId} className="text-xs font-medium bg-slate-100 text-slate-600 px-2.5 py-1.5 rounded-lg">{m.displayName}</span>
                ))}
              </div>
            </div>
          )}
...
function SummaryCard({ label, value, sub, danger, highlight }: { label: string; value: string; sub?: string; danger?: boolean; highlight?: boolean }) {
  const bg = danger ? "bg-red-50 border-red-200" : highlight ? "bg-violet-50 border-violet-200" : "bg-white border-slate-200/80";

  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-lg px-3 py-2.5 text-xs min-w-[160px]">
      <div className="flex items-center justify-between gap-3 mb-1.5">
        ...
        {chip && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-violet-100 text-violet-700">{chip}</span>}
      </div>
...
function GwNetChart({ gws, highlightedGw, onBarClick }: { gws: RegretGw[]; highlightedGw: number | null; onBarClick: (gw: number) => void }) {
    ...
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      <div className="px-4 pt-4 pb-3 border-b border-slate-100 flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Click a bar to jump to that gameweek · green = gain, red = loss</p>
        </div>
      ...
      <div className="px-2 py-3">
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={sorted} margin={{ top: 4, right: 12, left: -16, bottom: 0 }} barCategoryGap="22%">
            ...
            <YAxis domain={[-bound, bound]} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} allowDecimals={false} tickFormatter={(v) => (v > 0 ? `+${v}` : `${v}`)} />
            ...
            <Bar dataKey="gwNet" radius={[3, 3, 3, 3]} maxBarSize={44} style={{ cursor: "pointer" }}>
              {sorted.map((g, i) => {
                ...
                return <Cell key={i} fill={colour} opacity={highlightedGw === null || isHighlighted ? 1 : 0.35} onClick={() => onBarClick(g.gw)} />;
              })}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
...
function NetBadge({ net, size = "md" }: { net: number; size?: "sm" | "md" | "lg" }) {
  ...
  const zero = net === 0;
  const colour = positive ? "bg-emerald-100 text-emerald-700 border-emerald-200" : zero ? "bg-slate-100 text-slate-500 border-slate-200" : "bg-red-100 text-red-600 border-red-200";
  const sizeClass = size === "sm" ? "text-[10px] px-1.5 py-0.5" : size === "lg" ? "text-sm px-3 py-1.5 font-black" : "text-xs px-2 py-1";
  return (
    ...
    <div className="flex items-center gap-2 py-2.5 flex-wrap sm:flex-nowrap">
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className={`shrink-0 text-xs font-bold tabular-nums px-1.5 py-0.5 rounded-full border ${t.playerOut.pts >= t.playerIn.pts ? "bg-rose-100 text-rose-700 border-rose-200" : "bg-slate-100 text-slate-400 border-slate-200"}`}>
            {t.playerOut.pts} pts
          </span>
        </div>
      </div>
      <svg className="shrink-0 text-slate-300" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className={`shrink-0 text-xs font-bold tabular-nums px-1.5 py-0.5 rounded-full border ${t.playerIn.pts > t.playerOut.pts ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-400 border-slate-200"}`}>
            {t.playerIn.pts} pts
          </span>
        </div>
      </div>
      <div className="shrink-0 ml-auto"><NetBadge net={t.net} size="sm" /></div>
    </div>
  );
}
...
function GwBlock({ gw, highlighted }: { gw: RegretGw; highlighted?: boolean }) {
  ...
  return (
    <div id={`gw-block-${gw.gw}`} className={`bg-white rounded-xl overflow-hidden shadow-card transition-all duration-300 ${highlighted ? "border-2 border-[#37003c] ring-2 ring-[#37003c]/20" : "border border-slate-200/80"}`}>
      <div className={`flex items-center justify-between px-4 py-2.5 border-b border-slate-100 ${netIsPositive ? "bg-emerald-50/50" : netIsNegative ? "bg-red-50/40" : "bg-slate-50/60"}`}>
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-600 uppercase tracking-wide">GW{gw.gw}</span>
          {chip && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-violet-100 text-violet-700 border border-violet-200 uppercase tracking-wide">{chip}</span>}
          <span className="text-[10px] text-slate-400">{gw.transfers.length} transfer{gw.transfers.length !== 1 ? "s" : ""}</span>
        </div>
        <div className="flex items-center gap-2">
          {gw.hitCost < 0 && <span className="text-[10px] font-semibold text-red-500 tabular-nums">Hit {gw.hitCost} pts</span>}
          <NetBadge net={gw.gwNet} size="sm" />
        </div>
      </div>
      <div className="px-4 divide-y divide-slate-50">
        {gw.transfers.map((t, i) => <TransferRow key={i} t={t} />)}
      </div>
      {gw.transfers.length > 0 && (
        <div className={`px-4 py-2 border-t border-slate-50 text-xs font-medium ${netIsPositive ? "text-emerald-600" : netIsNegative ? "text-red-500" : "text-slate-400"}`}>
          {netIsPositive ? `Net gain: +${gw.gwNet} pts — good week in the market` : netIsNegative ? `Regret: ${gw.gwNet} pts — the tinker hurt` : "Dead even — break-even week"}
        </div>
      )}
    </div>
  );
            ...
            <div key={m.managerId} className="flex items-center gap-3 px-4 py-2.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${pos ? "bg-emerald-400" : neg ? "bg-red-400" : "bg-slate-300"}`} style={{ width: `${pct}%` }} />
                </div>
              </div>
            </div>
          );
...
function HallOfFame({ managers }: { managers: ManagerRegret[] }) {
  const allBest = managers.filter((m) => m.bestTransfer).map((m) => ({ m, t: m.bestTransfer! })).sort((a, b) => b.t.net - a.t.net);
  const allWorst = managers.filter((m) => m.worstTransfer).map((m) => ({ m, t: m.worstTransfer! })).sort((a, b) => a.t.net - b.t.net);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
        {overallBest && (
          <div className="px-4 py-4">
            <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wide mb-2 flex items-center gap-1.5"><span>🏆</span> Best Transfer</p>
            ...
            <div className="mt-2"><NetBadge net={overallBest.t.net} size="lg" /></div>
          </div>
        )}
        {overallWorst && (
          <div className="px-4 py-4">
            <p className="text-xs font-semibold text-red-500 uppercase tracking-wide mb-2 flex items-center gap-1.5"><span>💀</span> Worst Transfer</p>
            ...
            <div className="mt-2"><NetBadge net={overallWorst.t.net} size="lg" /></div>
          </div>
        )}
      </div>
...
export default function RegretPage() {
  ...
  const { data, isLoading, isError } = useQuery<RegretResponse>({
    queryKey: ["regret", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/regret`);
    ...
    <div className="space-y-5">
      ...
      {isLoading && (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => <div key={i} className="h-24 bg-white border border-slate-200/80 rounded-xl animate-pulse shadow-card" />)}
        </div>
      )}
      {isError && (
            ...
            {managers.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[10px] font-bold tabular-nums mt-0.5 ${isActive ? (pos ? "text-[#00ff87]" : neg ? "text-red-300" : "text-white/50") : (pos ? "text-emerald-600" : neg ? "text-red-500" : "text-slate-400")}`}>
          ...
          {active && (
            <div className="space-y-3">
              ...
              {active.gws.length === 0 ? (
                ...
                <>
                  <GwNetChart gws={active.gws} highlightedGw={highlightedGw} onBarClick={handleBarClick} />
                  {active.gws.map((gw) => <GwBlock key={gw.gw} gw={gw} highlighted={gw.gw === highlightedGw} />)}
                </>
              )}
            </div>
          )}

      ...
      {gw.isOptimal ? (
        ...
        <>
          <div className="flex justify-between gap-3 py-0.5">
            ...
            <span className="font-semibold text-slate-700 truncate max-w-[90px] text-right">{gw.actualCaptain.name} ({gw.actualCaptain.pts}pts)</span>
          </div>
          <div className="flex justify-between gap-3 py-0.5">
            <span className="text-slate-500">Best owned</span>
            <span className="font-semibold text-amber-700 truncate max-w-[90px] text-right">{gw.bestOwned.name} ({gw.bestOwned.pts}pts)</span>
          </div>
...
function MissedPtsChart({ gws }: { gws: WhatIfGw[] }) {
  ...
  function barColour(g: WhatIfGw): string {
    if (g.isOptimal) return "#10b981";
    if (g.missedPts <= 4) return "#f59e0b";
    if (g.missedPts <= 10) return "#f97316";
    return "#ef4444";
  }

  const optimalCount = chartData.filter((g) => g.isOptimal).length;
  const worstGw = chartData.reduce((b, g) => (g.missedPts > b.missedPts ? g : b), chartData[0]);
    ...
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      <div className="px-4 pt-4 pb-3 flex items-center justify-between gap-3 flex-wrap border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Bar height = raw pts missed · score impact is double</p>
        </div>
      ...
      <div className="px-2 py-3">
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={chartData} margin={{ top: 4, right: 12, left: -16, bottom: 0 }} barCategoryGap="20%">
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
            <XAxis dataKey="gw" tickFormatter={(v) => `GW${v}`} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
            <YAxis domain={[0, Math.ceil(maxMissed * 1.15)]} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} allowDecimals={false} />
            <Tooltip content={<MissedPtsTooltip />} cursor={{ fill: "#f8fafc" }} />
            <ReferenceLine y={0} stroke="#e2e8f0" strokeWidth={1} />
            <Bar dataKey="missedPts" radius={[3, 3, 0, 0]} maxBarSize={40}>
              {chartData.map((g, i) => <Cell key={i} fill={barColour(g)} opacity={g.isOptimal ? 0.9 : 1} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
...
function GwRow({ g }: { g: WhatIfGw }) {
  ...
  return (
    <div className={`px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 ${isGood ? "bg-emerald-50/40" : isBad ? "bg-red-50/30" : ""}`}>
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center gap-2 text-sm flex-wrap">
          ...
          <span className={`font-semibold ${isGood ? "text-emerald-700" : "text-slate-700"}`}>{g.actualCaptain.name}</span>
          <span className={`text-xs font-bold tabular-nums px-1.5 py-0.5 rounded-full border ${isGood ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-600 border-slate-200"}`}>
            {g.actualCaptain.pts} pts
          </span>
          {isGood && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-100 border border-emerald-200 px-1.5 py-0.5 rounded-full">✓ Optimal</span>}
        </div>
        {!isGood && (
          <div className="flex items-center gap-2 text-sm flex-wrap">
            ...
            <span className="text-xs font-bold tabular-nums px-1.5 py-0.5 rounded-full border bg-amber-100 text-amber-700 border-amber-200">{g.bestOwned.pts} pts</span>
          </div>
        )}
        {g.leagueBest && g.leagueBest.id !== g.bestOwned.id && (
          <div className="flex items-center gap-2 text-xs flex-wrap text-slate-400">
            <span className="font-semibold w-14 shrink-0">League best</span>
            <span>{g.leagueBest.name}</span>
            <span className="font-bold tabular-nums">{g.leagueBest.pts} pts</span>
            <span className="text-slate-300">({g.leagueBest.managerName})</span>
          </div>
        )}
      </div>
      <div className="shrink-0 text-right sm:min-w-[90px]">
        {isGood ? (
          ...
          <div>
            <div className="text-sm font-black tabular-nums text-red-600">−{g.missedPts} pts</div>
            <div className="text-[10px] text-slate-400 mt-0.5">−{g.actualImpact} to total</div>
          </div>
        )}
      </div>
    </div>
...
function LeagueRanking({ managers }: { managers: WhatIfManager[] }) {
  return (
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
        ...
        {managers.map((m, i) => {
          ...
          return (
            <div key={m.managerId} className="flex items-center gap-3 px-4 py-3 flex-wrap sm:flex-nowrap">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shrink-0 ${i === 0 ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-500"}`}>{i + 1}</div>
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden hidden sm:block">
                  <div className={`h-full rounded-full ${acc >= 60 ? "bg-emerald-400" : acc >= 30 ? "bg-amber-400" : "bg-red-400"}`} style={{ width: `${acc}%` }} />
                </div>
              </div>
            </div>
          );
...
export default function CaptainWhatIfPage() {
  ...
  const { data, isLoading, isError } = useQuery<WhatIfResponse>({
    queryKey: ["captain-whatif", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/captain-whatif`);
            ...
            {managers.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 flex flex-col items-start px-3 py-2 rounded-lg border text-left transition-all duration-150 ${isActive ? "bg-[#37003c] border-[#37003c] text-white shadow-md" : "bg-white border-slate-200 text-slate-600 hover:border-slate-300 shadow-card"}`}
                >
                  <span className="text-xs font-semibold truncate max-w-[90px]">{m.displayName}</span>
                  <span className={`text-[10px] font-bold tabular-nums mt-0.5 ${isActive ? "text-red-300" : "text-red-500"}`}>−{m.totalActualImpact} pts</span>
                </button>
              );
            })}
          </div>
          ...
          {active && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <SumTile label="Total pts lost" value={`−${active.totalActualImpact}`} sub="actual score impact" warn />
                <SumTile label="Raw missed" value={`−${active.totalMissedPts} pts`} sub="before ×2 multiplier" />
                <SumTile label="Optimal picks" value={`${active.optimalPicks} / ${active.gws.length}`} sub={`${acc}% accuracy`} highlight={acc >= 50} />
                {active.biggestMiss && (
                  <SumTile label="Biggest miss" value={`GW${active.biggestMiss.gw} (−${active.biggestMiss.actualImpact} pts)`} sub={`${active.biggestMiss.actualCaptain.name} → ${active.biggestMiss.bestOwned.name}`} warn />
                )}
              </div>
              ...
              <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
                ...
                {active.gws.length === 0 && <div className="py-12 text-center text-sm text-slate-400">No captain data available yet.</div>}
                <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between text-xs">
                  <span className="text-slate-400">{active.optimalPicks} optimal pick{active.optimalPicks !== 1 ? "s" : ""} this season</span>
                  <span className="text-red-500 font-semibold tabular-nums">Total impact: −{active.totalActualImpact} pts</span>
                </div>
              </div>
            </div>
          )}

          <LeagueRanking managers={managers} />

          <p className="text-xs text-slate-400 text-center px-2">
            &ldquo;Best owned&rdquo; = highest-scoring player in your 15-man squad that GW. &ldquo;Pts lost&rdquo; = raw difference × 2 (the actual impact on your total score).
          </p>
        </>
      )}
    </div>
...
function SumTile({ label, value, sub, highlight, warn }: { label: string; value: string; sub?: string; highlight?: boolean; warn?: boolean }) {
  return (
    <div className={`rounded-xl px-4 py-3 border shadow-card ${highlight ? "bg-emerald-50 border-emerald-200" : warn ? "bg-red-50 border-red-200" : "bg-white border-slate-200/80"}`}>
      <p className={`text-xs font-medium uppercase tracking-wider mb-1 ${highlight ? "text-emerald-500" : warn ? "text-red-400" : "text-slate-400"}`}>{label}</p>
      <p className={`text-sm font-black truncate ${highlight ? "text-emerald-700" : warn ? "text-red-600" : "text-slate-900"}`}>{value}</p>

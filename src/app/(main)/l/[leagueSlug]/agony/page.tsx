...
const COMPONENTS: { key: keyof AgonyBreakdown; label: string; colour: string; title: string }[] = [
  { key: "gwSuffering", label: "GW Suffering", colour: "bg-violet-500", title: "Sum of pts behind the league GW winner each week" },
...
function AgonyComponentChart({ managers }: { managers: AgonyManager[] }) {
  ...
  const leagueTotals = COMPONENTS.map(({ key, label }) => ({
    label,
    hex: COMPONENT_HEX[key],
    total: managers.reduce((s, m) => s + (m.breakdown[key] as number), 0),
  })).sort((a, b) => b.total - a.total);

  const dominantComponent = leagueTotals[0];
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-3 flex-wrap border-b border-slate-100">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">League Agony Breakdown</h2>
          <p className="text-xs text-slate-400 mt-0.5">Where the suffering comes from — stacked by component</p>
        </div>
        <div className="flex items-center gap-1.5 text-xs">
          <span className="text-slate-400">Biggest league pain:</span>
          <span
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={chartData} margin={{ top: 4, right: 16, left: -8, bottom: 0 }} barCategoryGap="28%">
            ...
            {COMPONENTS.map(({ key, label }, i) => (
              ...
              >
                {chartData.map((_, idx) => (
                  <Cell key={idx} fill={COMPONENT_HEX[key]} opacity={idx === 0 ? 1 : 0.85} />
                ))}
              </Bar>
            ))}
          </BarChart>
      ...
      <div className="px-4 pb-3 flex flex-wrap gap-x-5 gap-y-1.5">
        {leagueTotals.map(({ label, hex, total }) => (
...
function BreakdownBar({ breakdown, maxTotal }: { breakdown: AgonyBreakdown; maxTotal: number }) {
    ...
    <div>
      <div className="h-2 bg-slate-100 rounded-full overflow-hidden flex" style={{ width: `${barWidth}%`, minWidth: "4px" }}>
        {COMPONENTS.map(({ key, colour }) => {
          ...
          return pct > 0 ? <div key={key} className={`${colour} h-full`} style={{ width: `${pct}%` }} title={`${key}: ${val}`} /> : null;
        })}
      </div>
    </div>
  );
...
function AgonyRow({ manager, isFirst, isLast, maxTotal }: { manager: AgonyManager; isFirst: boolean; isLast: boolean; maxTotal: number }) {
    ...
    <div className={`px-4 py-4 ${isFirst ? "bg-red-50/40" : isLast ? "bg-emerald-50/30" : ""}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            ...
            <div className="shrink-0 text-right">
              <p className={`text-xl font-black tabular-nums leading-none ${isFirst ? "text-red-600" : isLast ? "text-emerald-600" : "text-slate-800"}`}>
      ...
      {manager.worstGw && manager.worstGw.orgWinner - manager.worstGw.pts >= 20 && (
        <div className="mt-3 ml-11 flex items-center gap-2 text-[11px] text-slate-400 bg-slate-50 border border-slate-100 rounded-lg px-3 py-1.5">
          <span>📅</span>
          <span>
            Worst GW: <strong className="text-slate-600">GW{manager.worstGw.gw}</strong> — scored {manager.worstGw.pts} pts while the GW winner got {manager.worstGw.orgWinner} pts
...
export default function AgonyPage() {
  ...
  const { data, isLoading, isError } = useQuery<AgonyResponse>({
    queryKey: ["agony", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/agony`);
        ...
        <>
          ...
          <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
            ...
            <div className="divide-y divide-slate-50">
              {managers.map((m, i) => (
                <AgonyRow key={m.managerId} manager={m} isFirst={i === 0} isLast={i === managers.length - 1} maxTotal={maxTotal} />
              ))}
            </div>
          </div>

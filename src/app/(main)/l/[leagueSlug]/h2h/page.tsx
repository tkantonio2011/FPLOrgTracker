...
function rivalrySummaryLine(a: H2HResponse["managerA"], b: H2HResponse["managerB"], s: H2HSummary): string {
...
function TrashTalkCard({ quoteA, quoteB, nameA, nameB, loading }: { quoteA: string | null; quoteB: string | null; nameA: string; nameB: string; loading: boolean }) {
  if (!loading && !quoteA && !quoteB) return null;
  return (
    <div className="rounded-xl overflow-hidden shadow-card border border-white/5" style={{ background: "linear-gradient(135deg, #0f0f0f 0%, #1a1a2e 50%, #0f0f0f 100%)" }}>
      <div className="px-4 py-2.5 border-b border-white/10 flex items-center justify-between">
        <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">🥊 Pre-Fight Weigh-In</span>
        {loading && (
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-white/30 animate-spin">
      ...
      <div className="grid grid-cols-2 min-h-[100px]">
        <div className="px-4 py-4 border-r border-white/10 flex flex-col gap-2">
          <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400/70">{nameA} corner</span>
          {loading && !quoteA ? (
        ...
        <div className="px-4 py-4 flex flex-col gap-2 items-end text-right">
          <span className="text-[9px] font-black uppercase tracking-widest text-rose-400/70">{nameB} corner</span>
          {loading && !quoteB ? (
      ...
      <div className="px-4 py-1.5 border-t border-white/5 text-center">
        <span className="text-[9px] text-white/15 font-medium">AI-generated banter</span>
      </div>
    </div>
  );
}

function ManagerSelect({ label, members, value, onChange, exclude }: { label: string; members: LeagueMember[]; value: number | null; onChange: (id: number) => void; exclude: number | null }) {
  return (
    <div className="flex-1 min-w-0">
      ...
      >
        ...
        {members.filter((m) => m.managerId !== exclude).map((m) => (
          <option key={m.managerId} value={m.managerId}>
            {m.displayName ?? `Manager ${m.managerId}`} · {m.teamName ?? ""}
          </option>
        ))}
      </select>
    </div>
...
function RivalryCard({ data }: { data: H2HResponse }) {
  ...
  return (
    <div className="bg-[#37003c] rounded-xl overflow-hidden shadow-card text-white">
      <div className="px-5 pt-5 pb-4 border-b border-white/10">
        <p className="text-[#00ff87] text-xs font-semibold uppercase tracking-wider mb-3">Rivalry Record</p>
        <div className="flex items-center gap-3">
          <div className="flex-1 min-w-0 text-center">
            ...
            <p className={`text-3xl font-black tabular-nums mt-2 leading-none ${leaderIsA ? "text-[#00ff87]" : isLevel ? "text-white/70" : "text-white/40"}`}>{s.winsA}</p>
          ...
          <div className="flex-1 min-w-0 text-center">
            ...
            <p className={`text-3xl font-black tabular-nums mt-2 leading-none ${leaderIsB ? "text-[#00ff87]" : isLevel ? "text-white/70" : "text-white/40"}`}>{s.winsB}</p>
            <p className="text-white/40 text-[10px] uppercase tracking-wide mt-0.5">wins</p>
          </div>
        </div>
      </div>
      <div className="px-5 py-3 border-b border-white/10">
        <p className="text-white/70 text-xs leading-relaxed">{rivalrySummaryLine(a, b, s)}</p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-white/10">
        <StatPill label="Avg margin" value={`${s.avgMargin} pts`} />
        <StatPill label="Net pts" value={s.netPtsA === 0 ? "Level" : `${s.netPtsA > 0 ? fnA : fnB} +${Math.abs(s.netPtsA)}`} />
        <StatPill label={`${fnA} best streak`} value={`${s.longestStreakA}W`} />
        <StatPill label={`${fnB} best streak`} value={`${s.longestStreakB}W`} />
      </div>
      {s.currentStreakHolder && s.currentStreak >= 2 && (
        <div className="px-5 py-2.5 bg-white/5 text-xs text-white/60 text-center">
          {s.currentStreakHolder === "A" ? fnA : fnB} is on a <span className="text-[#00ff87] font-bold">{s.currentStreak}-GW winning streak</span>
        </div>
      )}
    </div>
  );
...
function ScoresTooltip({ active, payload, label, fnA, fnB }: any) {
  if (!active || !payload?.length) return null;
  const a = payload.find((p: { name: string; value: number }) => p.name === "a")?.value ?? 0;
  const b = payload.find((p: { name: string; value: number }) => p.name === "b")?.value ?? 0;
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-lg px-3 py-2.5 text-xs min-w-[130px]">
      ...
      {margin > 0 && <p className="mt-1.5 pt-1.5 border-t border-slate-100 text-slate-500">{winner} wins by <span className="font-bold text-slate-700">{margin}</span></p>}
      {margin === 0 && <p className="mt-1.5 pt-1.5 border-t border-slate-100 text-slate-500">Draw</p>}
    </div>
  );
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function GapTooltip({ active, payload, label, fnA, fnB }: any) {
  if (!active || !payload?.length) return null;
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-lg px-3 py-2.5 text-xs min-w-[130px]">
      ...
      {ahead ? (
        <p className={gap > 0 ? "text-emerald-700 font-semibold" : "text-rose-600 font-semibold"}>{ahead} leads by <span className="font-bold">{abs} pts</span></p>
...
function H2HCharts({ data }: { data: H2HResponse }) {
  ...
  const chartData = gws.map((g) => {
    runA += g.ptsA;
    runB += g.ptsB;
    return { gw: g.gw, a: g.ptsA, b: g.ptsB, gap: runA - runB, winner: g.winner };
  });
  const COLOR_A = "#059669";
      ...
      <div className="flex border-b border-slate-100">
        {(["scores", "gap"] as const).map((tab) => (
          <button
            ...
            className={`flex-1 px-4 py-3 text-xs font-semibold transition-colors duration-150 ${activeTab === tab ? "text-violet-700 border-b-2 border-violet-500 bg-violet-50/40" : "text-slate-400 hover:text-slate-600 hover:bg-slate-50/60"}`}
          >
        ...
        {activeTab === "scores" && (
          <>
            <div className="flex items-center justify-center gap-5 mb-3 text-xs font-semibold">
              <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm inline-block" style={{ background: COLOR_A }} />{fnA}</span>
              <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm inline-block" style={{ background: COLOR_B }} />{fnB}</span>
            </div>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={chartData} barCategoryGap="30%" barGap={2} margin={{ top: 4, right: 12, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                <XAxis dataKey="gw" tickFormatter={(v) => `${v}`} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} label={{ value: "GW", position: "insideBottomRight", offset: -4, fontSize: 10, fill: "#cbd5e1" }} />
                <YAxis tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={30} />
                <Tooltip content={<ScoresTooltip fnA={fnA} fnB={fnB} />} cursor={{ fill: "#f8fafc" }} />
                <Bar dataKey="a" name="a" radius={[3, 3, 0, 0]}>
                  {chartData.map((d, i) => <Cell key={i} fill={d.winner === "A" ? COLOR_A : d.winner === "draw" ? COLOR_DRAW : `${COLOR_A}55`} />)}
                </Bar>
                <Bar dataKey="b" name="b" radius={[3, 3, 0, 0]}>
                  {chartData.map((d, i) => <Cell key={i} fill={d.winner === "B" ? COLOR_B : d.winner === "draw" ? COLOR_DRAW : `${COLOR_B}55`} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            <p className="text-center text-[10px] text-slate-300 mt-1">Faded bars = losing score · solid = winning score</p>
          </>
        )}
        {activeTab === "gap" && (
          <>
            <div className="flex items-center justify-center gap-5 mb-3 text-xs font-semibold">
              <span className="flex items-center gap-1.5"><span className="w-3 h-1.5 rounded inline-block" style={{ background: COLOR_A }} />{fnA} ahead</span>
              <span className="flex items-center gap-1.5"><span className="w-3 h-1.5 rounded inline-block" style={{ background: COLOR_B }} />{fnB} ahead</span>
            </div>
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={chartData} margin={{ top: 4, right: 12, left: -10, bottom: 0 }}>
                ...
                <Tooltip content={<GapTooltip fnA={fnA} fnB={fnB} />} cursor={{ stroke: "#e2e8f0", strokeWidth: 1 }} />
                <Line
                  type="monotone"
                  dataKey="gap"
                  stroke="#7c3aed"
            ...
            <p className="text-center text-[10px] text-slate-300 mt-1">Above zero = {fnA} leads · below zero = {fnB} leads</p>
          </>
        )}
      </div>
    </div>
    ...
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50/60">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-400">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-emerald-100 border border-emerald-300 inline-block" />{fnA} wins</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-red-100 border border-red-300 inline-block" />{fnB} wins</span>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {rows.map((g) => {
              ...
              const rowBg = isAWin ? "bg-emerald-50/60" : isBWin ? "bg-red-50/60" : "";
              return (
                <tr key={g.gw} className={rowBg}>
                  <td className="px-4 py-2.5 text-xs font-semibold text-slate-400 tabular-nums">GW{g.gw}</td>
                  <td className="px-4 py-2.5 text-right">
                    <span className={`text-sm font-bold tabular-nums ${isAWin ? "text-emerald-700" : isBWin ? "text-slate-400" : "text-slate-600"}`}>{g.ptsA}</span>
                  </td>
                  <td className="px-2 py-2.5 text-center">
                    {isAWin && <span className="inline-flex items-center justify-center text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 border border-emerald-200">+{g.margin}</span>}
                    {isBWin && <span className="inline-flex items-center justify-center text-[10px] font-bold px-1.5 py-0.5 rounded bg-red-100 text-red-600 border border-red-200">-{g.margin}</span>}
                    {g.winner === "draw" && <span className="inline-flex items-center justify-center text-[10px] font-semibold px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">Draw</span>}
                  </td>
                  <td className="px-4 py-2.5">
                    <span className={`text-sm font-bold tabular-nums ${isBWin ? "text-red-600" : isAWin ? "text-slate-400" : "text-slate-600"}`}>{g.ptsB}</span>
                  </td>
                  <td className="px-4 py-2.5 text-right hidden sm:table-cell">
                    <span className={`text-xs font-semibold tabular-nums ${g.runA > g.runB ? "text-emerald-600" : g.runB > g.runA ? "text-red-500" : "text-slate-400"}`}>{g.runA}–{g.runB}</span>
                  </td>
                </tr>
              );
            })}
      ...
      <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-2.5 flex items-center justify-between text-xs text-slate-500">
        <span>{gws.length} GW{gws.length !== 1 ? "s" : ""} played</span>
        <span className="tabular-nums font-semibold">{fnA} {a.totalPoints} pts · {fnB} {b.totalPoints} pts</span>
      </div>
    </div>
  );
}
...
export default function H2HPage() {
  ...
  const { data: membersData } = useQuery<MembersResponse>({
    ...
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/members`);
  ...
  const { data, isLoading, isError } = useQuery<H2HResponse>({
    queryKey: ["h2h", league.id, resolvedA, resolvedB],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/h2h?a=${resolvedA}&b=${resolvedB}`);
      const body = await r.json();
      if (!r.ok || !body.success) throw body;
      return body.data;
    },
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-slate-200/80 rounded-xl px-4 py-4 shadow-card">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <ManagerSelect label="Manager A" members={members} value={resolvedA} onChange={setIdA} exclude={resolvedB} />
          <div className="shrink-0 text-slate-300 font-black text-lg sm:mt-5">VS</div>
          <ManagerSelect label="Manager B" members={members} value={resolvedB} onChange={setIdB} exclude={resolvedA} />
        </div>
      </div>
      ...
      {(talkLoading || quoteA || quoteB) && data && (
        <TrashTalkCard quoteA={quoteA} quoteB={quoteB} nameA={firstName(data.managerA.displayName)} nameB={firstName(data.managerB.displayName)} loading={talkLoading} />
      )}
      ...
      {!enabled && members.length < 2 && (
        <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl text-sm">
          At least 2 active members are required for H2H.
        </div>
      )}
    </div>
  );

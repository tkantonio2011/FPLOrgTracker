...
function ChanceBar({ chance }: { chance: number | null }) {
  ...
  const colour = chance >= 75 ? "bg-emerald-400" : chance >= 50 ? "bg-amber-400" : chance >= 25 ? "bg-orange-500" : "bg-red-500";
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden max-w-[80px]">
        <div className={`h-full rounded-full ${colour}`} style={{ width: `${chance}%` }} />
      </div>
      <span className={`text-xs font-bold tabular-nums ${colour.replace("bg-", "text-")}`}>{chance}%</span>
    </div>
  );
}
        ...
        <div className="flex-1 min-w-0 space-y-2.5">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-2 flex-wrap">
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${POSITION_COLOUR[alert.elementType]}`}>{POSITION_LABEL[alert.elementType]}</span>
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
              ...
              <span className={`text-[10px] font-bold px-2 py-1 rounded-full border ${meta.colour}`}>{meta.label}</span>
            </div>
          </div>
          {alert.news && (
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                {alert.newsAdded && <p className="text-[10px] text-slate-400 mt-0.5">{formatNewsAge(alert.newsAdded)}</p>}
              </div>
            </div>
          )}
            ...
            {alert.owners.map((o) => (
              <span
                ...
                className={`text-[10px] font-medium px-2 py-1 rounded-full border whitespace-nowrap ${
                  o.isCaptain ? "bg-purple-50 text-purple-700 border-purple-200"
                    : o.isViceCaptain ? "bg-slate-100 text-slate-600 border-slate-200"
                    : o.isStarting ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                    : "bg-slate-50 text-slate-500 border-slate-200"
                }`}
              >
                {o.name}{o.isCaptain ? " ©" : o.isViceCaptain ? " V©" : !o.isStarting ? " (bench)" : ""}
              </span>
            ))}
          </div>
        </div>
...
export default function PlayerStatusPage() {
  ...
  const { data, isLoading, isError } = useQuery<PlayerStatusResponse>({
    queryKey: ["player-status", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/player-status`);
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <HelpButton topic="/help/reading-the-app/injuries" />
          <p className="text-sm text-slate-400 mt-0.5">
            {data ? `${data.gameweekName} · league-owned players with FPL flags` : "League-owned players with injury or availability concerns"}
          </p>
        </div>
        {data?.deadlineTime && (
      ...
      {isLoading && (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => <div key={i} className="h-32 bg-white border border-slate-200/80 rounded-xl animate-pulse shadow-card" />)}
        </div>
      )}
      {isError && (
      ...
      {data?.allClear && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-8 text-center shadow-card">
          ...
          <p className="text-sm text-emerald-700">No injury or doubt flags on any league-owned players right now.</p>
        </div>
      )}
          ...
          {outAlerts.length > 0 && (
            <section className="space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-red-700 uppercase tracking-wide">Out / Unavailable ({outAlerts.length})</h2>
              </div>
          ...
          {doubtAlerts.length > 0 && (
            <section className="space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-amber-700 uppercase tracking-wide">Doubtful ({doubtAlerts.length})</h2>
              </div>

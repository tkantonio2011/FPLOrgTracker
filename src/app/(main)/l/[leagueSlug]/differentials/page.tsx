  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 mb-1">
        <span className="text-xs font-bold text-slate-700 tabular-nums">{ownerCount}/{totalMembers}</span>
        <span className="text-xs text-slate-400">own</span>
      </div>
      <div className="h-2 rounded-full bg-slate-100 overflow-hidden flex">
        <div className="h-full bg-emerald-400 rounded-full transition-all duration-300" style={{ width: `${ownedPct}%` }} />
      </div>
      <div className="flex flex-wrap gap-x-2 gap-y-0.5 mt-1.5">
        {owners.map((n) => (
          <span key={n} className="text-[10px] font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded-full whitespace-nowrap">{n}</span>
        ))}
        {nonOwners.map((n) => (
          <span key={n} className="text-[10px] font-medium text-slate-500 bg-slate-50 border border-slate-200 px-1.5 py-0.5 rounded-full whitespace-nowrap">{n}</span>
        ))}
      </div>
    </div>
  );
...
export default function DifferentialsPage() {
  ...
  const { data, isLoading, isError } = useQuery<DifferentialsResponse>({
    queryKey: ["differentials", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/differentials`);
  ...
  const filtered = useMemo(() => {
    if (!data) return [];
    let list = posFilter === 0 ? data.differentials : data.differentials.filter((d) => d.elementType === posFilter);
    list = [...list].sort((a, b) => {
      ...
      if (sortKey === "owned") {
        const aSplit = Math.abs(a.leagueOwnerPercent - 50);
        const bSplit = Math.abs(b.leagueOwnerPercent - 50);
  ...
  const mostSplit = filtered.find((d) => Math.abs(d.leagueOwnerPercent - 50) < 15);
    ...
    <div className="space-y-5">
      ...
      {isLoading && (
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => <div key={i} className="h-28 bg-white border border-slate-200/80 rounded-xl animate-pulse shadow-card" />)}
        </div>
      )}
      {isError && (
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <SummaryCard label="Total differentials" value={String(data.differentials.length)} sub="partially-owned players" />
            <SummaryCard label="High-alert" value={String(highAlert)} sub="swing score ≥ 15" danger={highAlert > 0} />
            <SummaryCard label="Medium alert" value={String(medAlert)} sub="swing score 8–14" warn={medAlert > 0} />
            <SummaryCard label="Most divisive" value={mostSplit?.webName ?? "—"} sub={mostSplit ? `${mostSplit.ownerCount}/${mostSplit.totalMembers} own · ${mostSplit.team}` : "all aligned"} />
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200/60 rounded-xl px-4 py-3 text-xs text-slate-500 leading-relaxed">
            ...
            <span className="font-semibold text-slate-700">12 pt swing</span> between owners and non-owners that gameweek. The score peaks when ownership is exactly 50/50.
            <span className="ml-2 inline-flex gap-2">
              <span className="inline-flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" /> ≥15 high</span>
              <span className="inline-flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500 inline-block" /> 8–14 medium</span>
              <span className="inline-flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-300 inline-block" /> &lt;8 low</span>
            </span>
          </div>
            ...
            <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
              {([0, 1, 2, 3, 4] as PosFilter[]).map((pos) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all duration-150 ${posFilter === pos ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                >
          ...
          {filtered.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {visible.map((d) => {
                ...
                const borderColour = isHighAlert ? "border-red-200" : isMedAlert ? "border-amber-200" : "border-slate-200/80";
                const alertDot = isHighAlert ? "bg-red-500" : isMedAlert ? "bg-amber-400" : "bg-slate-300";
                return (
                  <div key={d.playerId} className={`bg-white border ${borderColour} rounded-xl px-4 py-4 shadow-card`}>
                    <div className="flex items-start gap-3">
                      <div className={`w-2.5 h-2.5 rounded-full shrink-0 mt-1.5 ${alertDot}`} />
                      <div className="flex-1 min-w-0 space-y-3">
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${POSITION_COLOUR[d.elementType]}`}>{POSITION_LABEL[d.elementType]}</span>
                            ...
                            <span className="text-xs text-slate-500">Form: <span className="font-semibold text-slate-700">{d.form.toFixed(1)}</span></span>
                          </div>
                          <div className="flex items-center gap-4 shrink-0">
                            {d.nextFixture && (
                              <div className="text-right">
                                <div className="flex items-center gap-1 justify-end mb-0.5">
                                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${FDR_COLOUR[d.nextFixture.fdr] ?? "bg-slate-200 text-slate-600"}`}>{d.nextFixture.fdr}</span>
                                  <span className="text-xs font-semibold text-slate-600">{d.nextFixture.isHome ? "" : "@"}{d.nextFixture.opponent}</span>
                                </div>
                        ...
                        <OwnershipBar ownerCount={d.ownerCount} totalMembers={d.totalMembers} owners={d.owners} nonOwners={d.nonOwners} />
                      </div>
                    </div>
                  </div>
                );
              ...
              {filtered.length > 12 && (
                ...
                >
                  {showAll ? "Show fewer" : `Show ${filtered.length - 12} more differentials`}
                </button>
              )}
            </div>
          )}
...
function SummaryCard({ label, value, sub, danger, warn }: { label: string; value: string; sub?: string; danger?: boolean; warn?: boolean }) {
  const bg = danger ? "bg-red-50 border-red-200" : warn ? "bg-amber-50 border-amber-200" : "bg-white border-slate-200/80";
  const valueColour = danger ? "text-red-600" : warn ? "text-amber-700" : "text-slate-900";
  return (

...
interface HistoryEntry {
  ...
  // Renamed from orgAvgPoints in the league-scoped contract; PointsChart still
  // reads `orgAvgPoints` so we map it client-side below.
  leagueAvgPoints: number | null;
}
...
function SeasonNarrativeCard({ leagueId, managerId, currentGw, leagueName }: { leagueId: string; managerId: string; currentGw: number | null; leagueName: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-[#1e0a24] via-[#2d1040] to-[#1a0d28] border border-[#5a3070]/40 shadow-lg">
      <div className="absolute top-3 left-4 text-[80px] leading-none font-black text-white/5 select-none pointer-events-none">&ldquo;</div>
      <div className="px-6 pt-5 pb-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-widest text-purple-300/70">Season Narrative</span>
          </div>
          <span className="text-[10px] text-purple-400/50 tabular-nums">GW{currentGw} edition</span>
        </div>
        {loading ? (
          ...
          <p className="text-sm text-white/80 leading-relaxed italic relative z-10">{narrative}</p>
        ) : null}
        {narrative && (
          <div className="mt-3 pt-2.5 border-t border-white/8 flex items-center justify-between">
            <span className="text-[10px] text-purple-400/40">Auto-generated · {leagueName}</span>
...
export default function MemberPerformancePage() {
  ...
  const { data: performance, isLoading: perfLoading, isError: perfError } = useQuery<PerformanceData>({
    queryKey: ["performance", league.id, managerId],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/members/${managerId}/performance`);
  ...
  const mostRecentGw = performance?.history && performance.history.length > 0
  ...
  const { data: squad, isLoading: squadLoading } = useQuery<SquadData>({
    queryKey: ["squad", league.id, managerId, activeGw],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/members/${managerId}/squad?gw=${activeGw}`);
      const body = await r.json();
      if (!r.ok || !body.success) {
        if (r.status === 403) {
          return { managerId: Number(managerId), gameweekId: activeGw ?? 0, activeChip: null, picks: [], code: "MANAGER_PRIVATE" } as SquadData;
        }
        throw body;
      }
      return body.data;
  ...
  // Adapt new `leagueAvgPoints` field back to `orgAvgPoints` for the existing PointsChart component.
  const historyForChart = performance?.history.map((h) => ({
    ...h,
    orgAvgPoints: h.leagueAvgPoints,
  })) ?? [];
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Link
          href={`/l/${leagueSlug}/suggestions/${managerId}`}
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            {performance.seasonSummary.bestGameweek && (
              <StatCard value={performance.seasonSummary.bestGameweek.points} label={`Best GW (GW ${performance.seasonSummary.bestGameweek.id})`} accent="green" />
            )}
            <StatCard value={performance.seasonSummary.totalBenchPoints} label="Bench Pts Wasted" accent="orange" />
          </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-card">
            ...
            <PointsChart history={historyForChart} />
          </div>

          <BenchSummary history={historyForChart} />

          {completedGws.length > 0 && (
            <div className="flex items-center gap-2">
              <label htmlFor="gw-select" className="text-xs font-medium text-slate-500">View squad for:</label>

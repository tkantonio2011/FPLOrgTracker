      ...
      {entries.map((e, i) => (
        <div key={e.name} className="flex items-center justify-between gap-3 py-0.5">
          ...
          <span className="font-bold tabular-nums text-slate-800 shrink-0">
            {e.value}
            {i > 0 && <span className="text-slate-400 font-normal ml-1">−{leader.value - e.value}</span>}
          </span>
        </div>
      ))}
    </div>
...
function CumulativeCaptainChart({ managers, gameweeks, currentGw }: { managers: ManagerCaptain[]; gameweeks: number[]; currentGw: number }) {
  ...
  const leaderId = managers.reduce(
    (best, m) =>
      ((lastRow?.[String(m.managerId)] as number) ?? 0) > ((lastRow?.[String(best?.managerId)] as number) ?? 0) ? m : best,
    managers[0],
  )?.managerId;

  const allVals = chartData.flatMap((row) => managers.map((m) => (row[String(m.managerId)] as number) ?? 0));
    ...
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      <div className="px-4 pt-4 pb-3 border-b border-slate-100 flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Running total of raw captain points across the season</p>
        </div>
        <div className="flex flex-wrap gap-x-4 gap-y-1 shrink-0">
          {managers.map((m, idx) => (
            <span key={m.managerId} className="flex items-center gap-1.5 text-xs text-slate-500">
              ...
              />
              {m.displayName}
              <span className="font-semibold text-slate-700 tabular-nums">{m.totalCaptainPts}</span>
            </span>
          ))}
        </div>
      </div>
      <div className="px-2 py-3">
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={chartData} margin={{ top: 4, right: 16, left: -16, bottom: 0 }}>
            ...
            <YAxis domain={[Math.max(0, yMin - pad), yMax + pad]} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} allowDecimals={false} />
            <Tooltip content={<CumulativeTooltip />} cursor={{ stroke: "#e2e8f0", strokeWidth: 1 }} />
            <ReferenceLine x={currentGw} stroke="#cbd5e1" strokeDasharray="4 2" strokeWidth={1.5} />
            {managers.map((m, idx) => (
              <Line
                key={m.managerId}
                type="monotone"
...
export default function CaptainHistoryPage() {
  ...
  const { data, isLoading, isError } = useQuery<CaptainResponse>({
    queryKey: ["captain-history", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/captain-history`);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <HelpButton topic="/help/reading-the-app/captain-history" />
        <p className="text-sm text-slate-400 mt-0.5">Who each manager captained each week and their running efficiency</p>
      </div>

      {isLoading && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[1, 2, 3, 4].map((i) => <div key={i} className="h-20 bg-white border border-slate-200/80 rounded-xl animate-pulse shadow-card" />)}
          </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
            ...
            <div className="divide-y divide-slate-50">
              {data.managers.map((m, idx) => {
                ...
                return (
                  <div key={m.managerId} className={`px-4 sm:px-5 py-3.5 ${isFirst ? "bg-emerald-50/40" : isLast ? "bg-red-50/30" : ""}`}>
                    <div className="flex items-center gap-3">
                      <span className="w-6 text-sm font-bold text-slate-400 shrink-0 tabular-nums">{idx + 1}</span>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-3 mb-1.5 flex-wrap">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="text-sm font-semibold text-slate-800 truncate">{m.displayName}</span>
                            {isFirst && <span className="text-[9px] font-bold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full uppercase tracking-wide shrink-0">Best captain</span>}
                            {isLast && <span className="text-[9px] font-bold bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full uppercase tracking-wide shrink-0">Worst captain</span>}
                          </div>
                          <div className="flex items-center gap-4 shrink-0 text-xs text-slate-500">
                            ...
                            <div className="text-right">
                              <span className="text-lg font-black text-slate-800 tabular-nums">{m.avgCaptainPts}</span>
                              <span className="text-xs font-normal text-slate-400 ml-0.5">avg</span>
                            </div>
                          </div>
                        </div>
                        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-500" style={{ width: `${barPct}%`, background: colour }} />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="px-5 py-2.5 border-t border-slate-100 bg-slate-50/40 flex flex-wrap gap-x-6 gap-y-1">
              {data.managers.map((m, idx) => (
                <span key={m.managerId} className="text-xs text-slate-400">
                  <span className="inline-block w-2 h-2 rounded-full mr-1 relative top-px" style={{ background: COLOURS[idx % COLOURS.length] }} />
                  {m.displayName}: <span className="font-semibold text-slate-600 tabular-nums">{m.totalCaptainPts}pts total</span>
                  {m.blanks > 0 && <span className="text-red-400 ml-1">({m.blanks} blank{m.blanks !== 1 ? "s" : ""})</span>}
                </span>
              ))}
            </div>
          </div>

          <CumulativeCaptainChart managers={data.managers} gameweeks={data.gameweeks} currentGw={data.currentGw} />

          <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60">
              <h2 className="text-sm font-semibold text-slate-700">Gameweek-by-Gameweek</h2>
              <p className="text-xs text-slate-400 mt-0.5">
                Green = best captain that GW · Red = worst · <span className="font-semibold">TC</span> = Triple Captain
              </p>
            </div>
              ...
              <table className="w-full text-xs border-collapse min-w-max">
                <thead>
                  <tr className="bg-slate-50/60 border-b border-slate-100">
                    <th className="sticky left-0 z-10 bg-slate-50 px-4 py-2.5 text-left font-semibold text-slate-500 uppercase tracking-wide border-r border-slate-100 min-w-[3rem]">GW</th>
                    {data.managers.map((m, idx) => (
                      <th key={m.managerId} className="px-3 py-2.5 text-left font-semibold text-slate-600 whitespace-nowrap min-w-[110px]">
                        <div className="flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full shrink-0" style={{ background: COLOURS[idx % COLOURS.length] }} />
                          {m.displayName}
                        </div>
                      </th>
                    ))}
                ...
                <tbody>
                  {data.gameweeks.map((gw) => {
                    ...
                    return (
                      <tr key={gw} className="border-b border-slate-50 hover:bg-slate-50/40 transition-colors">
                        <td className="sticky left-0 z-10 bg-white px-4 py-2.5 font-bold text-slate-600 border-r border-slate-100 tabular-nums">{gw}</td>
                        {data.managers.map((m) => {
                          const rec = getRecord(m, gw);
                          const bg = cellBg(rec?.rawPoints ?? null, rowPts);
                          if (!rec) return <td key={m.managerId} className="px-3 py-2.5 text-slate-300">—</td>;
                          return (
                            <td key={m.managerId} className={`px-3 py-2.5 ${bg}`}>
                              <div className="flex items-center gap-1.5 min-w-0">
                                <span className="font-semibold text-slate-700 truncate max-w-[60px]" title={rec.captainName}>{rec.captainName}</span>
                                {rec.chipUsed && <span className="text-[9px] font-bold bg-purple-100 text-purple-700 px-1 py-0.5 rounded shrink-0">TC</span>}
                              </div>
                              <div className="flex items-center gap-1 mt-0.5">
                                <span className={`font-black tabular-nums ${rec.rawPoints === null ? "text-slate-300" : rec.rawPoints <= 2 ? "text-red-500" : rec.rawPoints >= 12 ? "text-emerald-600" : "text-slate-700"}`}>
                                  {rec.rawPoints ?? "?"}
                                </span>
                                <span className="text-slate-400">pts</span>
                                {rec.rawPoints !== null && rec.multiplier === 3 && <span className="text-[9px] text-purple-500 font-bold">×3</span>}
                              </div>
                            </td>
                          );
                        })}
                  ...
                  <tr className="bg-slate-50/60 border-t border-slate-200">
                    <td className="sticky left-0 z-10 bg-slate-50 px-4 py-2.5 font-bold text-slate-500 border-r border-slate-100 uppercase text-[10px] tracking-wide">Avg</td>
                    {data.managers.map((m) => (
                ...
                {[...data.managers]
                  ...
                  .map((m) => (
                    <div key={m.managerId} className="px-4 py-3 flex items-center gap-3">
                      <div className="w-2 h-2 rounded-full shrink-0" style={{ background: COLOURS[data.managers.findIndex((mg) => mg.managerId === m.managerId) % COLOURS.length] }} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-700 truncate">{m.displayName}</p>
                        <p className="text-xs text-slate-400">GW{m.bestGw!.gw} — {m.bestGw!.captainName}</p>
                      </div>
                      <span className="text-base font-black text-emerald-600 tabular-nums shrink-0">{m.bestGw!.pts}pts</span>
                    </div>
                  ))}
              </div>
            </div>
                ...
                {[...data.managers]
                  ...
                  .map((m) => (
                    <div key={m.managerId} className="px-4 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-700 truncate">{m.displayName}</p>
                        <p className="text-xs text-slate-400">GW{m.worstGw!.gw} — {m.worstGw!.captainName}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <span className={`text-base font-black tabular-nums ${m.worstGw!.pts <= 2 ? "text-red-600" : "text-slate-600"}`}>{m.worstGw!.pts}pts</span>
                        {m.worstGw!.pts <= 2 && <p className="text-[9px] text-red-400 font-bold uppercase tracking-wide">blank</p>}
                      </div>
                    </div>
                  ))}
              </div>

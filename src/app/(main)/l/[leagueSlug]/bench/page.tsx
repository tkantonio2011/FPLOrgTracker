...
function heatStyle(pts: number, max: number): { background: string; color: string } {
  ...
  const light = Math.round(90 - 48 * t);
  return { background: `hsl(${hue}, ${sat}%, ${light}%)`, color: t > 0.55 ? "#fff" : "#374151" };
}

function BenchHeatmap({ managers, gameweeks }: { managers: ManagerBench[]; gameweeks: number[] }) {
  const [hoveredCell, setHoveredCell] = useState<{ managerId: number; gw: number } | null>(null);

  const maxPts = Math.max(...managers.flatMap((m) => m.gwData.map((d) => d.benchPts)), 1);

  const ptsByManagerGw = new Map(
    managers.map((m) => [m.managerId, new Map(m.gwData.map((d) => [d.gw, d.benchPts]))]),
  );

  const gwLeagueTotals = new Map(
    gameweeks.map((gw) => [gw, managers.reduce((s, m) => s + (ptsByManagerGw.get(m.managerId)?.get(gw) ?? 0), 0)]),
  );
  const maxGwTotal = Math.max(...Array.from(gwLeagueTotals.values()), 1);
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
      ...
      <div className="overflow-x-auto px-4 py-3">
        <div className="min-w-max space-y-px">
          <div className="flex items-end gap-px">
            ...
            {gameweeks.map((gw) => {
              const leagueTotal = gwLeagueTotals.get(gw) ?? 0;
              const isHot = leagueTotal / maxGwTotal > 0.6;
              return (
                <div key={gw} className="w-6 text-center shrink-0" title={`GW${gw} league total: ${leagueTotal} bench pts`}>
                  <span className={`text-[9px] font-semibold ${isHot ? "text-red-400" : "text-slate-300"}`}>{gw}</span>
                </div>
              );
            })}
          </div>
            ...
            return (
              <div key={m.managerId} className="flex items-center gap-px">
                <div className="shrink-0 w-28 pr-2">
                  <span className="text-xs font-medium text-slate-700 truncate block">{m.displayName.split(" ")[0]}</span>
                </div>
                {gameweeks.map((gw) => {
                  const pts = gwMap.get(gw) ?? 0;
                  const style = heatStyle(pts, maxPts);
                  const isHovered = hoveredCell?.managerId === m.managerId && hoveredCell?.gw === gw;
          ...
          <div className="flex items-center gap-px mt-1 pt-1 border-t border-slate-100">
            <div className="shrink-0 w-28 pr-2">
              <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">League total</span>
            </div>
            {gameweeks.map((gw) => {
              const total = gwLeagueTotals.get(gw) ?? 0;
              ...
              return (
                <div
                  ...
                  title={`GW${gw} league total: ${total} bench pts`}
                >
                  {total > 0 ? total : ""}
                </div>
              );
      ...
      <div className="px-5 py-2 border-t border-slate-50 text-[10px] text-slate-400 flex items-center gap-3">
        ...
        <span>Red GW numbers = painful week league-wide</span>
      </div>
    </div>
  );
}
...
export default function BenchPage() {
  ...
  const { data, isLoading, isError } = useQuery<BenchResponse>({
    queryKey: ["bench", league.id],
    queryFn: async () => {
      const r = await fetch(`/api/leagues/${league.id}/bench`);
  ...
  const perGwData = selectedM?.gwData.map((d) => ({ gw: d.gw, benchPts: d.benchPts, displayName: selectedM.displayName })) ?? [];
  ...
  return (
    <div className="space-y-5">
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <SummaryCard label="Total wasted (league)" value={`${totalSeasonBench} pts`} sub={`across ${data.gameweeks.length} gameweeks`} />
            <SummaryCard label="Most wasteful" value={mostWasteful?.displayName ?? "—"} sub={mostWasteful ? `${mostWasteful.totalBenchPts} pts wasted` : undefined} danger />
            <SummaryCard label="Most efficient" value={leastWasteful?.displayName ?? "—"} sub={leastWasteful ? `${leastWasteful.totalBenchPts} pts wasted` : undefined} highlight />
            <SummaryCard label="Worst single GW" value={globalWorstGw ? `GW${globalWorstGw.gw}` : "—"} sub={globalWorstGw ? `${globalWorstGw.manager} (${globalWorstGw.pts} pts)` : undefined} />
          </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1.5">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            {isFirst && <span className="text-[9px] font-bold bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full uppercase tracking-wide shrink-0">Bench king</span>}
                            {isLast && <span className="text-[9px] font-bold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full uppercase tracking-wide shrink-0">Most efficient</span>}
                          </div>
          ...
          {data.gameweeks.length >= 1 && <BenchHeatmap managers={data.managers} gameweeks={data.gameweeks} />}

          {data.gameweeks.length >= 2 && (
            <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-card">
              <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between flex-wrap gap-2">
                <h2 className="text-sm font-semibold text-slate-700">{view === "cumulative" ? "Cumulative Bench Points" : "Per-Gameweek Bench Points"}</h2>
                <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
                  <button onClick={() => setView("cumulative")} className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all duration-150 ${view === "cumulative" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>Cumulative</button>
                  <button onClick={() => setView("pergw")} className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all duration-150 ${view === "pergw" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>Per GW</button>
                </div>
              </div>
              <div className="p-4">
                {view === "cumulative" ? (
                  <>
                    <ResponsiveContainer width="100%" height={260}>
                      <LineChart data={cumulativeData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                        ...
                        <XAxis dataKey="gw" tickFormatter={(v) => `GW${v}`} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
                        <Tooltip content={<CumulativeTooltip managers={data.managers} />} />
                        {data.managers.map((m, idx) => (
                          <Line key={m.managerId} type="monotone" dataKey={String(m.managerId)} stroke={COLOURS[idx % COLOURS.length]} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                        ))}
                      </LineChart>
                    </ResponsiveContainer>
                    <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-3 px-1">
                      {data.managers.map((m, idx) => (
                        <div key={m.managerId} className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: COLOURS[idx % COLOURS.length] }} />
                  ...
                  <>
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <div className="relative">
                        ...
                        >
                          {data.managers.map((m) => <option key={m.managerId} value={m.managerId}>{m.displayName}</option>)}
                        </select>
                      ...
                      {selectedM && <span className="text-xs text-slate-400 ml-1">Total: <strong className="text-slate-700">{selectedM.totalBenchPts} pts</strong></span>}
                    </div>
                      ...
                      <BarChart data={perGwData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                        ...
                        <Bar dataKey="benchPts" radius={[3, 3, 0, 0]}>
                          {perGwData.map((entry, idx) => {
                            ...
                            return <Cell key={`cell-${idx}`} fill={isWorst ? "#dc2626" : baseColour} opacity={isWorst ? 1 : 0.75} />;
                          })}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                    {selectedM?.worstGw && (
                      <p className="text-xs text-slate-400 mt-2 text-center">
                        Worst GW highlighted in red — <span className="font-semibold text-red-600">GW{selectedM.worstGw.gw}: {selectedM.worstGw.benchPts} pts</span>
                      </p>
                    )}
                  </>
                )}
...
function SummaryCard({ label, value, sub, danger, highlight }: { label: string; value: string; sub?: string; danger?: boolean; highlight?: boolean }) {
  const bg = danger ? "bg-red-50 border-red-200" : highlight ? "bg-emerald-50 border-emerald-200" : "bg-white border-slate-200/80";

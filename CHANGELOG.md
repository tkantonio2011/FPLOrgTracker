...
- **UAT environment (004)** — A separate UAT EC2 instance runs the same Next.js build alongside production, gated by an env-var allow-list (`UAT_ALLOWED_EMAILS`) and visually marked with a yellow banner on every page. The magic-link sign-in route enforces the allow-list while preserving the existing enumeration-resistant response shape (FR-009/010); non-allow-listed addresses receive the same `{ sent: true }` response with no DB write and no email sent. UAT pages emit `X-Robots-Tag: noindex, nofollow` from the Next.js middleware. Operator scripts under `scripts/uat/` (deploy, refresh-from-prod, snapshot, rollback, smoke-test) reuse `scripts/deploy.sh` via override env vars — no duplicated deploy code. UAT is provisioned by the same Terraform module behind `var.enable_uat` (default `true`), with `terraform output uat_public_ip` exposing the new host. See `specs/004-uat-deployment/` for the spec, plan, contracts, and runbook.

### Breaking Changes
- **Multi-tenant rebuild** — The app is no longer a single-organisation tracker; the singleton `Organisation` becomes a multi-tenant `League`, with full data isolation between leagues. Every member-facing page now lives under `/l/{leagueSlug}/...` instead of at the root; platform-wide administration lives at `/platform/...` and is restricted to Super Admins. Bookmarks pointing at `/standings`, `/admin`, etc. are automatically redirected to the equivalent league-scoped URL while the redirect shims are in place. Sharing a URL with someone who is not a member of that league returns 404 (deliberate non-disclosure, verified by the SC-006 isolation test suite).
- **Passwordless sign-in via magic-link** — `User` + password hashes are gone; sign-in is now a single email field at `/sign-in` that emails a one-time link valid for 15 minutes. Sessions are 30-day sliding cookies signed with HMAC. The legacy PIN-based admin flow and the registration form have been deleted (`refactor: T033 closure — delete legacy auth + admin surface`); there is no overlap window after the upgrade — every existing user must sign in via magic-link. Requires working SMTP (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`) in production; when SMTP is unset in dev the link is printed to the server console.
- **Three-tier role model** — `Member` (read), `League Admin` (manages a single league), `Super Admin` (manages the platform). Each route handler under `/api/leagues/[leagueId]/` is now gated by `requireLeagueMember` / `requireLeagueAdmin`; platform routes are gated by `requireSuperAdmin`. Every administrative action is written to a per-league audit log surfaced at `/l/{slug}/admin/audit`.
- **Industry-neutral copy** — All energy-trading / company-specific strings have been removed from member-facing surfaces; every league now configures its own name and logo from `/l/{slug}/admin/settings`.

### New Features
- **`/platform` Super Admin console** — Dedicated console for platform operators to create leagues, manage cross-league memberships, suspend or reinstate leagues, and review the global audit trail. League creation issues an invitation to the nominated League Admin's email and automatically generates a unique URL slug.
- **Invitations** — Admins invite members by email from `/l/{slug}/admin/members`; the invitee lands at `/invitations/{token}`, sees the league logo + name, optionally fills in their FPL Manager ID, accepts, and is signed in straight to `/l/{slug}/standings`. Invitations are single-use and expire after 7 days; the email field can be pre-supplied at invitation time so members map cleanly to an FPL manager.
- **Slug history with automatic redirects** — Renaming a league preserves the old slug in `LeagueSlugHistory` so existing bookmarks continue to resolve via `league-resolver.ts`. The redirect lands the user on the new canonical URL.
- **Multi-league member switching** — The top-bar `LeagueSwitcher` lists every active league the user belongs to, with an "Admin" chip on leagues where they hold that role; selecting a league navigates to its equivalent surface. When switching as an admin between admin sub-paths (e.g. `/l/A/admin/members` → League B), the helper `mapAdminPath` preserves the sub-shell so admins keep their context; switching to a league where the user is only a member falls back to `/standings`.
- **`/my-admin` admin home (Phase 8)** — Users who administer two or more leagues now have a dedicated page listing every league they manage, with deep-link buttons for Settings / Members / Digest / Audit per league. Suspended leagues remain listed with a "Suspended" chip and disabled deep-links. The sidebar surfaces a "My admin leagues" link inside any league shell once the user holds the role on 2+ leagues.
- **Partitioned `/leagues` chooser** — When a user belongs to leagues in both roles, the chooser splits them into "Leagues you administer" and "Leagues you're a member of"; users with leagues in only one role see the previous single-list layout unchanged.
- **Sidebar league badge** — Under the platform brand block, the sidebar now shows the current league's logo and name on any `/l/{slug}/...` page when a Logo URL is configured in `/l/{slug}/admin/settings`. Sources the badge data from the existing `/api/auth/me` fetch — no extra request.
- **Freshness gate on live / ownership / standings** — A new `useFreshnessGate` hook + `RefreshingPill` component surfaces an inline "refreshing…" pill when cached data is stale, so users no longer see yesterday's standings silently for a beat before the new fetch lands. Particularly noticeable on the standings page mid-deadline.
- **League-scoped digest, suggestions, narrative, tribunal, horoscope** — Every analytics / LLM endpoint that previously read an implicit "current organisation" now takes `leagueId` as an explicit parameter and reads `League.name` + `digestPrompt` from a per-request `LeagueContext`. The digest email and email-footer link respect the migrated league's name and configured `APP_URL`.

### Improvements
- **Industry-neutral default copy** in branding constants (`lib/branding/strings.ts`), removing the legacy energy-trading flavour from suspended-league screens, error messages, and helper text.
- **Zod-validated input at every boundary** — No untyped `req.json()` reaches the repository layer; route handlers parse and narrow before the data layer is touched, returning standardised `ok` / `fail` envelopes from `lib/http/response.ts`.
- **Audit logging on every administrative mutation** — Slug renames, member role changes, league suspension/reinstatement, invitation issuance, and the data migration itself all write to `AuditEvent`. Each league's audit log is visible at `/l/{slug}/admin/audit`; the Super Admin console exposes the global view at `/platform/audit`.
- **Per-route league-scoped caching** — TanStack Query keys now include the league id, so navigating between leagues doesn't bleed cached data across tenants. Server-side cache headers (`buildCacheHeader`) respect live-gameweek vs. historical-gameweek TTLs.
- **Sidebar groups + accordion** — Navigation is grouped into Gameweek / Season / Scout sections with an accordion that auto-opens to the group containing the active page; mobile drawer behaviour preserved.

### Bug Fixes
- **Landing-page quote card hydration mismatch** — `Math.random()` inside a `useState` initialiser produced a different quote on the server vs. the client's first render, so React occasionally aborted hydration with a `Text content does not match server-rendered HTML` error when clicking into a league from `/my-admin`; the initial index is now `0` and a `useEffect` randomises after mount, so SSR and the first client render emit identical HTML.
- **Standings page crash on partial responses** — `standingsData?.standings.slice(0, 5)` would throw `Cannot read properties of undefined (reading 'slice')` if the cached query result was a partial object (`standingsData` defined but `standings` missing); the queryFn now validates `Array.isArray(body.data.standings)` before accepting a response, and every consumer optional-chains through `.standings` so a malformed cache entry can no longer crash the page.
- **Magic-link `redirectTo` rejecting `null`** — The `/api/auth/magic-link` route's Zod schema rejected `null` for the optional `redirectTo` field, breaking the sign-in form when no return URL was supplied; the schema now accepts `null` and the link issuer omits the field from the email URL when it is empty.
- **Next.js instrumentation hook disabled** — Application bootstrap (Platform singleton creation, Super Admin upsert from env) silently never ran because `experimental.instrumentationHook` was off; enabling it in `next.config.js` restores the boot-time idempotent setup.
- **Legacy `/admin` not redirecting** — The pre-002 `/admin` shortcut returned 404 instead of redirecting to the relevant league's admin shell; it now routes to `/leagues` for multi-league users and to `/l/{slug}/admin/settings` for single-league admins. `/leagues` itself is Super-Admin aware and shows a footer link to `/platform` when applicable.
- **Member route slug collision** — Two dynamic segments named `[managerId]` and `[memberId]` under the same path produced a Next.js build error; unified on `[managerId]` everywhere.

### Migration & Operations
- **Expand-then-contract migration (`prisma/migrations/002_multi_league/`)** — `migration.sql` adds the new platform/league/auth tables without touching legacy `Organisation` / `Member` / `User`; `seed.ts` is idempotent and copies the existing singleton organisation across, creates `UserAccount`s for every member with an email, promotes the bootstrap League Admin, and writes a `migration.completed` audit event. Re-running the seed is a no-op. The contract migration (drop legacy tables) is deferred to a follow-up release.
- **Rollback path documented** — `prisma/migrations/002_multi_league/rollback.md` describes the restore-from-backup procedure. Every existing deployment must take `prisma/prod.db.pre-002` before applying the schema; there is no reverse-migration script (deleting `UserAccount` / `Session` rows would discard live sign-in state).
- **New bootstrap env vars** — `BOOTSTRAP_SUPER_ADMIN_EMAIL` (required in production, read on every boot, idempotent), `BOOTSTRAP_LEAGUE_ADMIN_EMAIL` (used by the seed only — remove after the 002 cutover). The pre-002 `ADMIN_PIN` and any password-related secrets can be removed.
- **E2E + integration test harness** — `tests/e2e/setup-db.ts` seeds a dedicated `prisma/e2e-test.db` with two leagues, one membership, and one signed-in session; `playwright.config.ts` pins `DATABASE_URL` to it and runs the dev server on port 3100 with `reuseExistingServer: false`, so the developer's real DB is never touched. New specs cover route-level league isolation (SC-006), member non-disclosure, and the multi-admin UX flow.
- **`.gitignore` extended** — `*.db-shm` (SQLite WAL shared-memory files) and `*.db.pre-*` (pre-migration backups) are now ignored so the prod backup file doesn't accidentally land in git.
